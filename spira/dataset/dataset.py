...
class Dataset(Dataset):
    ...
    def __init__(self, c, ap, wavs: list[WAV], classes: pd.Series, noise):
        assert (len(wavs) == len(classes), "Datasets and classes should have the same lengths")
    ...
    def _calculate_max_seq_length(self):
        ...
        print("The Max Time dim length is: {} (+- {} seconds)".format(max_len, (
                max_len * self.c.audio['hop_length']) / self.ap.sample_rate))
        print("The Min Time dim length is: {} (+- {} seconds)".format(min_len, (
                min_len * self.c.audio['hop_length']) / self.ap.sample_rate))
        return max_len
    ...
    # TODO: Decide if _insert_noise is a good function name
    def _insert_noise(self, wav: WAV, idx, class_name) -> WAV:
        ...
        if self.test:
            self._apply_random_seed(self.c['seed'] * idx) # multiply by idx, because if not we generate same some for all files !

        return self._add_noise_wav_to_input_wav(wav, class_name)

    # TODO: Decide if apply_random_seed is a good function name
    ...
    # TODO: Decide if _add_noise_wav_to_wav is a good function name
    def _add_noise_wav_to_input_wav(self, wav: WAV, class_name) -> WAV:
    ...
    def _calculate_noise_wav(self, wav: WAV) -> WAV:
    ...
    def _get_feature_and_target(self, wav: WAV, class_name):
    ...
    def _get_feature_and_target_using_overlapping(self, wav, class_name):
        features, targets = self._append_features_and_targets_using_window_len(wav, class_name)
